...
function locationToPort(location: Location, countryCode?: string): Port | null {
  ...
  const unlocode = String(location.unlocode ?? raw.locode ?? raw.location_code ?? "").trim().toUpperCase();
  const code = countryCodeFromRecord(raw) || unlocode.slice(0, 2);
  const fn = String(location.function ?? location.functions ?? location.function_code ?? raw.function_code ?? "").replace(/\s+/g, "");
