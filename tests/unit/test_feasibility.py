...
def test_feasibility_service_rejects_missing_cargo():
    cargo_id = uuid4()
    vessel_id = uuid4()
    origin_port_id = uuid4()
    destination_port_id = uuid4()
    ...
    service = FeasibilityService()
    service.cargo_repository = FakeRepository()
    service.vessel_repository = FakeRepository()
    service.constraint_repository = FakeRepository()
    service.compatibility_repository = FakeRepository()
    service.feasibility_repository = FakeRepository()
    ...
    try:
        ...
        assert False, "Expected missing cargo to raise ValueError"
    except ValueError as exc:
        assert str(exc) == "Cargo requirement not found."


def test_feasibility_service_rejects_missing_vessel():
    cargo_id = uuid4()
    vessel_id = uuid4()
    origin_port_id = uuid4()
    destination_port_id = uuid4()
    ...
    service = FeasibilityService()
    service.cargo_repository = FakeRepository({str(cargo_id): cargo})
    service.vessel_repository = FakeRepository()
    service.constraint_repository = FakeRepository()
    service.compatibility_repository = FakeRepository()
    service.feasibility_repository = FakeRepository()
    ...
    try:
        service.evaluate(payload)
        assert False, "Expected missing vessel to raise ValueError"
    except ValueError as exc:
        assert str(exc) == "Vessel not found."
